...
async function createClinic(req, res) {
  try {
    ...
    // Check if admin already has 10 clinics
    const [clinicCount] = await db.execute(
      'SELECT COUNT(*) as count FROM clinics WHERE is_active = 1'
    );

    if (clinicCount[0].count >= 10) {
      return res.status(400).json({ error: 'Maximum limit of 10 clinics reached. Cannot add more clinics.' });
    }
